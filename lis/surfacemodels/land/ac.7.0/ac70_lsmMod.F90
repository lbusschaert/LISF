...
! control the operation of Ac70 model. It also provides the entry method
! for the initialization of Ac70-specific variables. The derived
! data type {\tt Ac70\_struc} includes the variables that specify
! the runtime options and other control variables as described below:
!
! \begin{description}
! \item[rfile]
!  name of the Ac70 restart file
! \item[rformat]
!  format of restart file (binary or netcdf) for Ac70
...
! \item[ts]
!   Ac70 model time step in second
! \item[count]
...
>>>>>>> 1022b9963b1190fa8126b34ae308640ad42abe98
    ...
    type(Ac70_type_dec), pointer :: AC70_struc(:)
...
! !DESCRIPTION:
!
!  This routine creates the datatypes and allocates memory for Ac70-specific
!  variables. It also invokes the routine to read the runtime specific options
!  for Ac70 from the configuration file.
!
!  The routines invoked are:
!  \begin{description}
!   \item[Ac70\_readcrd](\ref{Ac70_readcrd}) \newline
!    reads the runtime options for Ac70 model
!  \end{description}
!EOP
        implicit none        
        integer  :: n, t     
        ...
        allocate(AC70_struc(LIS_rc%nnest))
        ...
        do n=1, LIS_rc%nnest
            ...
            allocate(AC70_struc(n)%ac70(LIS_rc%npatch(n, LIS_rc%lsm_index)))
            !------------------------------------------------------------------------
            ...
            do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
                allocate(AC70_struc(n)%ac70(t)%shdfac_monthly(12))
                allocate(AC70_struc(n)%ac70(t)%smceq(AC70_struc(n)%nsoil))
            enddo
...
!            allocate(AC70_struc(n)%init_smc(AC70_struc(n)%nsoil))
            !allocate(AC70_struc(n)%init_zss( AC70_struc(n)%nsoil + AC70_struc(n)%nsnow))
            !allocate(AC70_struc(n)%init_snowice(AC70_struc(n)%nsnow))
            !allocate(AC70_struc(n)%init_snowliq(AC70_struc(n)%nsnow))
            ...
            !   AC70_struc(n)%ac70(t)%sfhead1rt = 0.0
                AC70_struc(n)%ac70(t)%infxs1rt = 0.0
                AC70_struc(n)%ac70(t)%soldrain1rt = 0.0
#endif
                
                AC70_struc(n)%ac70(t)%albd = -9999.0
                AC70_struc(n)%ac70(t)%albi = -9999.0
                AC70_struc(n)%ac70(t)%alb_upd_flag = .false.
                !ag(05Jan2021)
                AC70_struc(n)%ac70(t)%rivsto = 0.0
                AC70_struc(n)%ac70(t)%fldsto = 0.0
                AC70_struc(n)%ac70(t)%fldfrc = 0.0
...
!------------------------------------------------------------------------
            ...
            call LIS_update_timestep(LIS_rc, n, AC70_struc(n)%ts)

            call LIS_registerAlarm("Ac70 model alarm",&
                                   AC70_struc(n)%ts, &
                                   AC70_struc(n)%ts)

            call LIS_registerAlarm("Ac70 restart alarm", &
                                   AC70_struc(n)%ts,&
                                   AC70_struc(n)%rstInterval)
!------------------------------------------------------------------------
