...
!   25 Feb 2022   Mahdi Navari; added LIS_getCDFtransferattributes
...
!-----------------------------------------------------------------------------
  ...
  public :: LIS_rescale_with_normal_deviate_scaling
  public :: LIS_rescale_with_linear_scaling
  public :: LIS_rescale_with_anomaly        !kyh20210422
  ...
  public :: LIS_getCDFtransferattributes
  public :: LIS_rescale_with_stratified_CDF 
...
!BOP
!
! !ROUTINE: LIS_rescale_with_normal_deviate_scaling
! \label{LIS_rescale_with_normal_deviate_scaling}
!
! !INTERFACE:
  subroutine LIS_rescale_with_normal_deviate_scaling(&
       n,             &
       k,             &
       ntimes,        &
       model_mu,      &
...
! !ARGUMENTS:
    ...
    real                :: model_sigma(LIS_rc%obs_ngrid(k),ntimes)
    real                :: obs_mu(LIS_rc%obs_ngrid(k),ntimes)
    real                :: obs_sigma(LIS_rc%obs_ngrid(k),ntimes)
    real                :: obs_value(LIS_rc%obs_lnc(k),LIS_rc%obs_lnr(k))
!
! !DESCRIPTION:
!
...
!EOP
    ...
    do t=1,LIS_rc%obs_ngrid(k)
       ...
       if(obs_value(col,row).ne.-9999.0.and.&
            model_sigma(t,kk).gt.0.and.&
            obs_sigma(t,kk).gt.0) then

          obs_tmp = (obs_value(col,row) - obs_mu(t,kk))*model_sigma(t,kk)/&
               obs_sigma(t,kk) + model_mu(t,kk)
  ...
  end subroutine LIS_rescale_with_normal_deviate_scaling
...
!BOP
! 
! !ROUTINE: LIS_rescale_with_anomaly
! \label{LIS_rescale_with_anomaly}
!
! !INTERFACE:
  subroutine LIS_rescale_with_anomaly(&
       n,             &
       k,             &
       nbins,         &
       ntimes,        &
...
!BOP
! 
! !ROUTINE: LIS_rescale_with_stratified_CDF
! \label{LIS_rescale_with_stratified_CDF}

! !REVISION HISTORY:
! 3 March 2022: Mahdi Navari ; Initial Specification
!
! !INTERFACE:
  subroutine LIS_rescale_with_stratified_CDF(&
       n,             &
       k,             &
       nbins,         &
       ntimes,        &
...
! !ARGUMENTS: 
    ...
    real                :: max_obs_value
    real                :: min_obs_value
    real                :: model_xrange(n_strat_bins,ntimes, nbins)
    real                :: obs_xrange(n_strat_bins,ntimes, nbins)
...
!   This routine rescales the input observation data to the model's
!   climatology so that the cumulative distribution functions (CDFs)
!   of the observations and the model match (But we use geolocation 
!   independent CDF, therefore we match the input observation data to 
!   model's climatology for each stratified bins not for each grid point). 
! 
!  The arguments are: 
!  \begin{description}
!  \item[n]               index of the nest
...
!EOP
    ...
    real                :: model_delta(n_strat_bins)
    real                :: obs_delta(n_strat_bins)
    real                :: ref_p_min, ref_p_max,delta_p_ref
    integer             :: t,i,kk
    integer             :: binval, strat_binval
    integer             :: col,row
    real                :: cdf_obsval
    real                :: obs_tmp, obs_in
    integer, dimension (1)      :: index_25 , index_75
...
!BOP
! !ROUTINE: LIS_getCDFtransferattributes
! \label{LIS_getCDFtransferattributes}

! !REVISION HISTORY:
! 3 March 2022: Mahdi Navari ; Initial Specification
!
! !INTERFACE: 
   subroutine LIS_getCDFtransferattributes(k,filename, n_strat_bins, ntimes)
...
! 
! !DESCRIPTION: 
!  This routine reads the number of stratification bins abd other CDF attribute from the 
!  CDF file
! 
!  The arguments are: 
!  \begin{description}
...
!EOP
     ...
     call LIS_verify(nf90_inq_dimid(nid, 'n_strat_bins',binId), &
          'Error nf90_inq_dimid: n_strat_bins')

     call LIS_verify(nf90_inquire_dimension(nid, binId, len=n_strat_bins), &
          'Error nf90_inquire_dimension:n_strat_bins')
   ...
   end subroutine LIS_getCDFtransferattributes
...
! \label{read_CDFtransferdata_all}
! !REVISION HISTORY: 
! 1 March 2022: Mahdi Navari ; Initial Specification
!
! !INTERFACE: 
   subroutine read_CDFtransferdata_all(n, k, nbins, ntimes, n_strat_bins, &
        filename, varname, xrange, cdf)
...
! !ARGUMENTS:
     ...
     integer,   intent(in)    :: nbins
     integer,   intent(in)    :: ntimes
     integer,   intent(in)    :: n_strat_bins
     character(len=*)         :: filename
     character(len=*)         :: varname
     real                     :: xrange(n_strat_bins,ntimes, nbins)
     real                     :: cdf(n_strat_bins,ntimes, nbins)

! 
! !DESCRIPTION: 
!  This routine reads the input CDF file (generated by LDT in NETCDF format)
!  The xrange values and the corresponding CDFs are read for each stratification bin. 
! 
!  The arguments are: 
!  \begin{description}
!  \item[n]             index of the nest
...
!EOP
     integer                  :: j,kk
     integer                  :: nstratbinId, nbinsId, nlevsId
     integer                  :: nstratbin_file, nbins_file, nlevs_file
     integer                  :: xid, cdfid
     real, allocatable        :: xrange_file(:,:,:,:)
     real, allocatable        :: cdf_file(:,:,:,:)
     integer                  :: nid
...
!EOP
  ...
  if(file_exists) then
     ...
     call LIS_verify(ios,'Error in nf90_inq_dimid in read_precip_climo')

     ios = nf90_inq_dimid(nid,"north_south",nrId)
     call LIS_verify(ios,'Error in nf90_inq_dimid in read_precip_climo')

     ios = nf90_inquire_dimension(nid,ncId, len=gnc)
     call LIS_verify(ios,'Error in nf90_inquire_dimension in read_precip_climo')

     ios = nf90_inquire_dimension(nid,nrId, len=gnr)
     call LIS_verify(ios,'Error in nf90_inquire_dimension in read_precip_climo')

     if (gnc .ne. LIS_rc%gnc(n) .or. gnr .ne. LIS_rc%gnr(n)) then
        write(LIS_logunit,*) '[ERR] domain dimension mismatch. Number of rows and columns in the '
        write(LIS_logunit,*) 'precipitation climatology file differs from that in the LIS domain '
        call LIS_endrun()
     endif
     ...
     do i=1,12
        ...
        call LIS_verify(ios,'Precipitation climo field not found in the file')

        ios = nf90_get_var(nid,varId,precip(:,:,i)) ! precip_climo(:,:,i))
        call LIS_verify(ios,'Error in nf90_get_var in read_Precip_climo')
     enddo

     ios = nf90_close(nid)
     call LIS_verify(ios,'Error in nf90_close in read_Precip_climo')
...
!EOP
  ...
  if(file_exists) then
     ...
     ios = nf90_inq_dimid(nid,"east_west",ncId)
     call LIS_verify(ios,'Error in nf90_inq_dimid in read_precip_climo_maxval')

     ios = nf90_inq_dimid(nid,"north_south",nrId)
     call LIS_verify(ios,'Error in nf90_inq_dimid in read_precip_climo_maxval')

     ios = nf90_inquire_dimension(nid,ncId, len=gnc)
     call LIS_verify(ios,'Error in nf90_inquire_dimension in read_precip_climo_maxval')
     ...
     do i=1,12
        ...
        call LIS_verify(ios,'Error in nf90_get_var in read_Precip_climo_maxval')
     enddo

     ios = nf90_close(nid)
     call LIS_verify(ios,'Error in nf90_close in read_Precip_climo_maxval')
     write(LIS_logunit,*) '[INFO] Successfully read Precipitation climo file ',trim(filename)
  endif
...
end subroutine read_Precip_climo_maxval
